...
function Admin() {
	...
	Admin.prototype.validatePassword = function(enteredPassword, callback) {
		db.query("SELECT value FROM settings WHERE name = 'salt';", function(err, rows) {
			if (err) { logger.error("Unable to get password-salt: " + err.code); callback(false, err.code); }
			var salt = rows[0].value;
		
			db.query("SELECT value FROM settings WHERE name = 'password';", function(err, rows) {
				if (err) { logger.error("Unable to save new website-availability: " + err.code); callback(false, err.code); }
	...
	Admin.prototype.changePassword = function(newPassword, callback) {
		...
		db.query("UPDATE settings SET value = ? WHERE name = 'salt';", [ salt ], function(err, result) {
			if (err) {
				logger.error("Unable to change password-salt: " + err.code);
				callback(false, err.code);
			} else {
				db.query("UPDATE settings SET value = ? WHERE name = 'password';", [ password ], function(err, result) {
					if (err) {
	...
	Admin.prototype.addAdmin = function(newPassword, callback) {
		...
		db.query("INSERT INTO settings SET ?;", insertData, function(err, result) {
			if (err) {
