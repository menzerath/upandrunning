	...
	Website.prototype.runCheck = function() {
		request({ url: this.protocol + '://' + this.url, headers: { 'User-Agent': 'UpAndRunning (https://github.com/MarvinMenzerath/UpAndRunning)' } }, function (error, response, body) {
			...
			if (status == 200 || status == 301 || status == 302) {
				// success
				db.query("UPDATE website SET status = ?, time = NOW(), ups = ups + 1, totalChecks = totalChecks + 1 WHERE id = ?;", [ status + ' - ' + httpcodes[status], id ], function(err, result) {
					if (err) { logger.error("Unable to save new website-status: " + err.code); }
				});
			} else {
				// failure
				db.query("UPDATE website SET status = ?, time = NOW(), lastFailStatus = ?, lastFailTime = NOW(), downs = downs + 1, totalChecks = totalChecks + 1 WHERE id = ?;", [ status + ' - ' + httpcodes[status], status + ' - ' + httpcodes[status], id ], function(err, result) {
					if (err) { logger.error("Unable to save new website-status: " + err.code); }
				});
			}
