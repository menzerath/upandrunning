...
router.get('/websites/add/:name/:protocol/:url', function (req, res) {
	...
	db.query("INSERT INTO website SET ?;", insertData, function (err) {
		...
		} else { res.send({ requestSuccess: true }); }
	});
});
...
router.get('/settings/interval/:interval', function (req, res) {
	...
	if (isNaN(newInterval) || newInterval < 1 || newInterval > 60) { res.status(400).send({ requestSuccess: false, message: 'Unable to process your request: Interval has to be a number ranging between 1 and 60 minutes' }); return; }
